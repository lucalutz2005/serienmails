...
#for EMAIL in $(echo "use serienmails; SELECT user_email FROM einladungen WHERE antwort IS NULL" |  mysql -h localhost -u serienmails --password="garnix" | grep -v "name" | grep -v "email" ); do
 ...
 #       ZIP="zip /home/luca/serienmails/webgui/smt/$EVENT.zip /home/luca/serienmails/webgui/adressen/$EVENT/*"
  #      echo $ZIP | bash;
#done
#echo "Sir hie die Zip die sie Anforderten"  | mailx -s "ZIP" -A /home/luca/serienmails/webgui/smt/$1.zip lucas.soundstudio@outlook.de
